...
// Handles signup, login, and token rotation. Stores users in NOTE_KV.
...
export async function onRequestPost(context) {
  ...
  const { action, username, password, oldToken } = body;
  ...
  if (action === 'signup') {
    ...
    await env.NOTE_KV.put(`token:${token}`, JSON.stringify({
      username: cleanUsername, text: '', updatedAt: null
    }));
    ...
    // Verify old token matches this user
    const stored = await env.NOTE_KV.get(userKey, { type: 'json' });
    if (!stored || stored.token !== oldToken) {
      return Response.json({ error: 'Unauthorized' }, { status: 401 });
    }
    // Migrate note content to new token
